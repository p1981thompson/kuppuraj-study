...
mixed3<-lmer(RT~ Block+ TP + (Block-1|ID), data=mm_data)
...
newparams <- list(
  beta = structure(c(817.150000000002, -23.29, -63.34), .Names = c("(Intercept)", "Block", "TPPR")), #fixed effects (intercept, Block, TPPR)
  theta = getME(mixed3, "theta"),#Random slope
  sigma = getME(mixed3, "sigma"))#random slope variance

# Simulate:

ss <- simulate(~ Block + TP + (Block-1|ID), nsim = nsim, newdata = expdat, newparams = newparams,family=gaussian)#syntax "block-1" means we don't use random intercepts for block.
...
fitsim3 <- function(i) {
  ...
  expdat$RT <- ss[, i]
  new1<-lmer(RT~ Block + TP +(Block-1|ID),data=expdat, REML = FALSE, control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
