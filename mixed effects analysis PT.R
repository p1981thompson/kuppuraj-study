...
for (i in 1:nsubs){
  ...
  slopedata[i,1] = as.character(unique(rawdata$ID)) 
  slopedata[i,2] = unique(rawdata$Gender)
  slopedata[i,3] = unique(rawdata$Age)
  slopedata[i,4] = as.character(unique(rawdata$Group))
  ...
  meds1$Dis_TP<-car::recode(meds1$Type,"1='nd_DT';2='ad_DT';3='nd_PR';4='ad_PR'")
...
mm_data$Block<-as.numeric(mm_data$Block)

mm_data$ID<-as.factor(mm_data$ID)
mm_data$Dis_TP<-as.factor(mm_data$Dis_TP)
mm_data$Dis_TP<-factor(mm_data$Dis_TP,levels=c("ad_DT", "nd_DT", "ad_PR", "nd_PR"))

mm_data$Dist<-as.factor(mm_data$Dist)
