#################################################################################
#
# Power calculation for linear mixed effects model (kuppu) - version 3
#
#################################################################################

# Created by Paul Thompson, 05-05-2017

#http://stackoverflow.com/questions/38316308/lmer-get-p-values-from-anova
...
library(lme4) # mixed models package for R.
library(car)
#library(nlme)
library(lmerTest) #absolutely must have this package as it gives p-values output for the lmer() model objects.

#run model for Kuppu's pilot data using NLME and LME4. Need both as lme function produces p-values but lmer does not.

mixed4<-lmer(RT~ Block + TP + Dist + (1+Block|ID), data=mm_data2, REML=FALSE)
...
N=10
nsim<-100
expdat <- expand.grid(ID = factor(1:N), Block = factor(1:5), Type=factor(1:4), Set=factor(1:10))
...
expdat$Set<-as.numeric(expdat$Set)
...
newparams <- list(
  beta = fixef(mixed4), #fixed effects (intercept, Block, TPPR)
  theta = getME(mixed4, "theta"),#Random slope
  sigma = getME(mixed4, "sigma"))#random slope variance

# Simulate:

ss <- simulate(~ Block + TP + Dist +(1+Block|ID), nsim = nsim, newdata = expdat, newparams = newparams,family=gaussian)#syntax "block-1" means we don't use random intercepts for block.
...
fitsim3 <- function(i) {
  ...
  expdat$RT <- ss[, i]
  new1<-lmer(RT~ Block + TP + Dist +(1+Block|ID),data=expdat, REML = FALSE, control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
