...
#simulates and calculates power for 2(groups as within sub factor), 2 (dependency types as within sub factor),
...
nPerGroup <- 25 # total participants per group 
nDepType  <- 2 #levels in the first within sub factor 
nPredictability<-2 #levels in the first within sub factor 
RTAdult   <- c(0.79,0.35,1.04,0.66) # means taken from the Pilot data of 5 adults-in the order DEpType:Predic, 1 1, 1 2, 2 1, 2 2  
RTChild    <- c(.792,0.1,.6,.3) # estimated values (children)
stdevs    <- rep(1,4) # It is the average of deviations across the 18 values mentioned above,
#kept it constant to avoid the Sigma being negative-which gives error. 
stdiff    <- 0.4  #sphericity. 
