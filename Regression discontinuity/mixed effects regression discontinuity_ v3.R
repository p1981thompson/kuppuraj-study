...
main.data2$broke0<-ifelse(main.data2$SetInd %in% c(1:24),1,0)
main.data2$broke1<-ifelse(main.data2$SetInd %in% c(25:32),1,0) 
...
# mod1e <- lmer(log_TargetRT ~ Type + b1(SetInd_c, bp1) + b2(SetInd_c, bp1,bp2) + b3(SetInd_c,bp2) + Type*b1(SetInd_c, bp1) + Type*b2(SetInd_c, bp1,bp2) + Type*b3(SetInd_c, bp2) +(0+b1(SetInd_c, bp1) + b2(SetInd_c, bp1,bp2) + b3(SetInd_c, bp2)| ID)+(0+broke0+broke1+broke2|ID), data = main.data2, REML = TRUE,control = lmerControl(optimizer = "optimx", calc.derivs = TRUE, optCtrl = list(method = "nlminb")))

mod1e2 <- lmer(log_TargetRT ~ Type + b1(SetInd_c, bp1) + b2(SetInd_c, bp1,bp2) + b3(SetInd_c,bp2) + Type*b1(SetInd_c, bp1) + Type*b2(SetInd_c, bp1,bp2) + Type*b3(SetInd_c, bp2) +(0+b1(SetInd_c, bp1) + b3(SetInd_c, bp2)| ID)+(0+broke0+broke1+broke2|ID), data = main.data2, REML = TRUE,control = lmerControl(optimizer = "optimx", calc.derivs = TRUE, optCtrl = list(method = "nlminb")))
...
newdat1d$broke0<-ifelse(newdat1d$SetInd %in% unique(main.data2$SetInd_c)[1:24],1,0)
newdat1d$broke1<-ifelse(newdat1d$SetInd %in% unique(main.data2$SetInd_c)[25:32],1,0)
newdat1d$broke2<-ifelse(newdat1d$SetInd %in% unique(main.data2$SetInd_c)[33:40],1,0) 

newdat2d$broke0<-ifelse(newdat2d$SetInd %in% unique(main.data2$SetInd_c)[1:24],1,0)
newdat2d$broke1<-ifelse(newdat2d$SetInd %in% unique(main.data2$SetInd_c)[25:32],1,0)
newdat2d$broke2<-ifelse(newdat2d$SetInd %in% unique(main.data2$SetInd_c)[33:40],1,0) 

newdat3d$broke0<-ifelse(newdat3d$SetInd %in% unique(main.data2$SetInd_c)[1:24],1,0)
newdat3d$broke1<-ifelse(newdat3d$SetInd %in% unique(main.data2$SetInd_c)[25:32],1,0) 
newdat3d$broke2<-ifelse(newdat3d$SetInd %in% unique(main.data2$SetInd_c)[33:40],1,0) 
  ...
  geom_line(data=newdat1d,aes(y=predict(mod1e2,newdata=newdat1d)),size = .75)+
    geom_line(data=newdat2d,aes(y=predict(mod1e2,newdata=newdat2d)),size = .75)+
    geom_line(data=newdat3d,aes(y=predict(mod1e2,newdata=newdat3d)),size = .75)+
...
plot(mod1e,type=c("p","smooth"))

plot(mod1e,sqrt(abs(resid(.)))~fitted(.),
                  type=c("p","smooth"),ylab=expression(sqrt(abs(resid))))

plot(mod1e,resid(.,type="pearson")~SetInd, type=c("p","smooth"))

qqnorm(resid(mod1e))
qqline(resid(mod1e))

hist(resid(mod1e),100)
