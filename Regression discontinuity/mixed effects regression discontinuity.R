...
mod1c <- lmer(TargetRT ~ Type + b1(SetInd, bp1) + b2(SetInd, bp1,bp2) + b3(SetInd,bp2) + Type*b1(SetInd, bp1) + Type*b2(SetInd, bp1,bp2) + Type*b3(SetInd, bp2) 
            + (b1(SetInd, bp1) + b2(SetInd, bp1,bp2) + b3(SetInd, bp2)| ID), data = main.data, REML = FALSE, control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

newdat1c<-expand.grid(Type=unique(main.data$Type),SetInd=1:40,ID=unique(main.data$ID))
  ...
  geom_line(data=newdat1c,aes(y=predict(mod1c,newdata=newdat1c)),size = .75)+
